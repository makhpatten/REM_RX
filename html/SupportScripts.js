...
function CallbackNetburner()
{
	...
	reqObjEverything.open('get','ajaxReload.htm'); //invoke the desired function on the netburner
	...
	reqObjTxStatus.open('get','ajaxTxStatus.htm'); //invoke the desired function on the netburner
	//by sending the htm page that has the <!--FUNCTIONCALL AjaxCallbackTxStatus -->
	//The netburner method AjaxCallback() is in webformcode.cpp
	//Tell the ProcessAjaxResponse method to handle the returned data
...
function ProcessAjaxResponseTxStatus()
{
	...
	if((reqObjTxStatus.readyState == REQUEST_DONE)&& (reqObjTxStatus.status == STATUS_NORMAL))
	{
		...
		//ajaxText is the id of a DIV tag in index.htm
		document.getElementById('ajaxTxStatus').innerHTML = answer;
	}
	else
	{
//		document.getElementById('ajaxTxStatus').innerHTML = "Error in ProcessAjaxResponseTxStatus";
	}
}
